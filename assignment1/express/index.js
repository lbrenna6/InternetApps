 ...
 * 1: No city could be extracted from the req.params object - usually means the user has made a mistake 
 * 2: The city provided in params failed the regex test - contains something other than alphabet characters 
 * 3: There was a problem making the open weather api request - done inside callWeatherAPI function
 */
    ...
    try {
        ...
        if (response.status === 200){
            const data = response.data;         //data = the data object OpenWeatherMap returns
                ...
                const weatherList = data.list.filter((element, index) => index % 8 === 0);
